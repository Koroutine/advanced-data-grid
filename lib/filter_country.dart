...
class _FilterCountryState extends State<FilterCountry> {
  ...
  CountryCode _selectedCountry = CountryCode.fromCountryCode("GB");
  ...
  void initState() {
    ...
    if (filters.isNotEmpty) {
      if (widget.filterType == DataFilterType.COUNTRY_NAME) {
        setState(() {
          _selectedCountry = CountryCode.fromJson({"name": filters[0].value!});
        });
      } else {
        setState(() {
          _selectedCountry = CountryCode.fromCountryCode(filters[0].value!);
        });
      }
    }
  ...
  void save() {
    ...
    if (widget.filterType == DataFilterType.COUNTRY_NAME) {
      filters.add(DataFilter(operator: null, value: _selectedCountry.name));
    } else {
